...
namespace MEI.Web.Areas.Admin.Pages.Users
{
    public class IndexModel : PageModel
    {
        ...
        {
            var groupQuery = new GetGroupsQuery();
            var groups = await _queries.Execute(groupQuery);
            Groups = GetGroupList(groups);
        }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var userQuery = new FindByGroupQuery()
            {
                GroupName = SelectedGroup.Trim()
            };
            var members = await _queries.Execute(userQuery);
            // Convert the AD user into Table record
            Members = members.Select(m => new MemberTableItem(m, _options.DefaultUserAvatarImagePath)).Where(m => m.Enabled == true).ToList();
