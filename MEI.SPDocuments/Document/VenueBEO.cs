...
{
    [DocumentInfo(SPDocumentType.VenueBEO, "VenueBEO", "VBEO", "VenueBEO", "Venue BEO")]
    public class VenueBEO
        : SPDocumentBase, ISearchVendor, ISearchProgram, ISearchYear
    {
        internal VenueBEO(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public VenueBEO WithValues(string programId, int? vendorId)
        {
            ProgramId = programId;
            VendorId = vendorId;
