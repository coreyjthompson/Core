...
{
    [DocumentInfo(SPDocumentType.Receipt, "Receipt", "RCT", "Receipt")]
    public class Receipt
        : SPDocumentBase, ISearchExpense, ISearchProgram, ISearchVendor, ISearchYear
    {
        internal Receipt(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public Receipt WithValues(string programId, int? expenseCounter)
        {
        ...
        {
            if ((result.Status == SPActionStatus.Success) && ((Company == Company.Abbott) | (Company == Company.ExactSciences))
                                                          && DocumentYear.Compare(DocumentYear.Year2014, ">="))
            {
                Repository.InsertIntoReceiptsOnReceiptUpload(ProgramId, ExpenseCounter, Company, DocumentYear);
            }
