...
{
    [DocumentInfo(SPDocumentType.BanquetEventOrder, "BanquetEventOrder", "BEO", "BEO", "Banquet Event Order", "BEO")]
    public class BanquetEventOrder
        : SPDocumentBase, ISearchVendor, ISearchProgram, ISearchYear
    {
        internal BanquetEventOrder(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public BanquetEventOrder WithValues(string programId, int? vendorId)
        {
            ProgramId = programId;
            VendorId = vendorId;
