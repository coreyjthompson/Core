...
{
    [DocumentInfo(SPDocumentType.VenueContract, "VenueContract", "VC", "VenueContract", "Venue Contract")]
    public class VenueContract
        : SPDocumentBase, ISearchVendor, ISearchProgram, ISearchYear
    {
        internal VenueContract(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public VenueContract WithValues(string programId, int? vendorId)
        {
            ProgramId = programId;
            VendorId = vendorId;
