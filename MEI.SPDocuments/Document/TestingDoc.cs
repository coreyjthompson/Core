...
{
    [DocumentInfo(SPDocumentType.TestingDoc, "TestingDoc", "TD", "TD", "Testing Doc")]
    public class TestingDoc
        : SPDocumentBase, ISearchProgram, ISearchYear
    {
        internal TestingDoc(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public TestingDoc WithValues(string programId)
        {
            ProgramId = programId;
