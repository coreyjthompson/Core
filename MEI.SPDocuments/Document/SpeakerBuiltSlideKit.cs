...
{
    [DocumentInfo(SPDocumentType.SpeakerBuiltSlideKit, "SpeakerBuiltSlideKit", "SBSK", "SBSK", "Speaker Built Slide Kit", "SBSK")]
    public class SpeakerBuiltSlideKit
        : SPDocumentBase, ISearchProgram, ISearchYear, ISearchSpeaker
    {
        internal SpeakerBuiltSlideKit(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public SpeakerBuiltSlideKit WithValues(string programId, int? speakerCounter)
        {
            ProgramId = programId;
        ...
        public override int ActionAfterUpload(UploadResult result)
        {
            ...
            return Repository.UpdateSpeakerBuiltSlideKitOnUpload(ProgramId, Convert.ToInt32(SpeakerCounter), DocumentYear);
        }
    }
}
