...
{
    [DocumentInfo(SPDocumentType.EPPoster, "EPPoster", "EPPSTR")]
    public class EPPoster
        : SPDocumentBase
    {
        internal EPPoster(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public EPPoster WithValues(int? posterId, EPassStatus statusTypeCode)
        {
            PosterId = posterId;
        ...
        [SPFieldInfo(SPFieldNames.PosterId, "PosterID", SPFieldType.Text, 0)]
        ...
        public override string FileName => MakeFileName(PosterId, Status.ToDisplayNameShort());
        ...
        public override string UniqueIdentifiers => "PosterId;Status";

        public override string UniqueValues => string.Format("{0};{1}", PosterId, Status.ToDisplayNameLong());
        ...
        public override bool Setup(object[] objects)
        {
            ...
            PosterId = Convert.ToInt32(objects[0]);
        ...
        {
            if (values.ContainsKey(SPFields[SPFieldNames.PosterId].InternalName))
            {
                PosterId = Convert.ToInt32(values[SPFields[SPFieldNames.PosterId].InternalName]);
            }
        ...
        public override IDictionary<string, string> GetUserFieldValues()
        {
            return new Dictionary<string, string>
                   {
                       { SPFields[SPFieldNames.PosterId].InternalName, PosterId.ToString() },
        ...
        public override string[] ParseFileName(string fileNameToParse)
        {
            ...
            if (!int.TryParse(fileNameParts[1], out int tempPosterId))
            {
                ThrowFileNameExceptionInvalidType(fileNameToParse, SPFieldNames.PosterId, "Integer");
            }
