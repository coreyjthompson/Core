...
{
    [DocumentInfo(SPDocumentType.TravelAgencyServiceInvoice, "TravelAgencyServiceInvoice", "TASINV")]
    public class TravelAgencyServiceInvoice
        : SPDocumentBase, ISearchProgram, ISearchYear
    {
        internal TravelAgencyServiceInvoice(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public TravelAgencyServiceInvoice WithValues(string programId, int speakerId, int invoiceId)
        {
            ProgramId = programId;
            SpeakerCounter = speakerId;
        ...
        [SPFieldInfo(SPFieldNames.InvoiceNumber, "InvoiceNumber", SPFieldType.Integer, 2)]
        public int InvoiceNumber { get; set; }

        public override DocumentYear DocumentYear => ExtractDocumentYear(ProgramId);

        public override string FileName => MakeFileName(InvoiceNumber, SpeakerCounter, ProgramId);
        ...
        public override bool Setup(object[] objects)
        {
            ...
            InvoiceNumber = Convert.ToInt32(objects[0]);
        ...
        public override bool AbstractSetup(Hashtable values)
        {
            ...
            {
                InvoiceNumber = (int)values[SPFields[SPFieldNames.TravelDocId].InternalName];
            }

            if (values.ContainsKey(SPFields[SPFieldNames.TramsHistoryId].InternalName))
            {
                SpeakerCounter = (int)values[SPFields[SPFieldNames.TramsHistoryId].InternalName];
            }
        ...
        public override IDictionary<string, string> GetUserFieldValues()
        {
            return new Dictionary<string, string>
                   {
                       { SPFields[SPFieldNames.TravelDocId].InternalName, InvoiceNumber.ToString() },
                       { SPFields[SPFieldNames.ProgramId].InternalName, ProgramId },
                       { SPFields[SPFieldNames.TramsHistoryId].InternalName, SpeakerCounter.ToString() }
                   };
        }
        ...
        public override string[] ParseFileName(string fileNameToParse)
        {
            ...
            InvoiceNumber = Convert.ToInt32(fileNameParts[1]);
