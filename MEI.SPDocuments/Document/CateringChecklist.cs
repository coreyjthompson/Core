...
{
    [DocumentInfo(SPDocumentType.CateringChecklist, "CateringChecklist", "CCL", "CCL", "Catering Checklist")]
    public class CateringChecklist
        : SPDocumentBase, ISearchVendor, ISearchProgram, ISearchYear
    {
        internal CateringChecklist(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public CateringChecklist WithValues(string programId, int? vendorId)
        {
            ProgramId = programId;
            VendorId = vendorId;
