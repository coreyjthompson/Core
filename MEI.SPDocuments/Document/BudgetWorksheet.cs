...
{
    [DocumentInfo(SPDocumentType.BudgetWorksheet, "BudgetWorksheet", "BW", "BudgetWorksheet", "Budget Worksheet")]
    public class BudgetWorksheet
        : SPDocumentBase, ISearchProgram, ISearchYear
    {
        internal BudgetWorksheet(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public BudgetWorksheet WithValues(string programId)
        {
            ProgramId = programId;
