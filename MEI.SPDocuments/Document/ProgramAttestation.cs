...
{
    [DocumentInfo(SPDocumentType.ProgramAttestation, "ProgramAttestation", "PA", "PA", "Program Attestation")]
    public class ProgramAttestation
        : SPDocumentBase, ISearchProgram, ISearchYear
    {
        internal ProgramAttestation(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public ProgramAttestation WithValues(string programId)
        {
            ProgramId = programId;
        ...
        {
            if (((Company == Company.Abbott) | (Company == Company.ExactSciences)) && DocumentYear.Compare(DocumentYear.Year2017, "=>")
                                                                                   && (result.Status == SPActionStatus.Success))
            {
                Repository.CreateTicklersOnProgramAttestationUpload(ProgramId, Company, DocumentYear);
            }
