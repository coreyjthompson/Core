...
{
    [DocumentInfo(SPDocumentType.TravelReceipt, "TravelReceipt", "TRCT")]
    public class TravelReceipt
        : SPDocumentBase, ISearchProgram, ISearchYear
    {
        internal TravelReceipt(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public TravelReceipt WithValues(string programId, int travelDocId, int tramsHistoryId)
        {
            ProgramId = programId;
        ...
        [SPFieldInfo(SPFieldNames.InvoiceNumber, "TicketNumber", SPFieldType.Text, 3)]
            ...
            return new Dictionary<string, string>
                   {
                       ...
                       { SPFields[SPFieldNames.TicketNumber].InternalName, TicketNumber }
                   };
        }
        ...
        public override string[] ParseFileName(string fileNameToParse)
        {
            ...
            //' Lookup ticket number from TravelDocSearch table
            TicketNumber = Repository.GetTicketNumberForTravelDocId(TravelDocId);
