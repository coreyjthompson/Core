...
{
    [DocumentInfo(SPDocumentType.PSAAddendum, "PSAA")]
    public class PSAAddendum
        : SPDocumentBase, ISearchYear, ISearchProgram
    {
        internal PSAAddendum(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public PSAAddendum WithValues(string agreementId, int? speakerCounter, string speakerName)
        {
        ...
        private string GetProgramIDForPSA(string agreementIdToSearch)
        {
            ...
            try
            {
                ...
                var p1 = new SqlParameter("@SpkrAgmtID", SqlDbType.VarChar, 50)
                         {
                             Direction = ParameterDirection.Input,
                             Value = agreementIdToSearch
                         };
                cmd.Parameters.Add(p1);
                var p2 = new SqlParameter("@ProgramId", SqlDbType.VarChar, 20)
                         {
