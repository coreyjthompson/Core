...
{
    [DocumentInfo(SPDocumentType.ProgramChangesTravel, "ProgramChangesTravel", "PCT", "PCT", "Program Changes Travel")]
    public class ProgramChangesTravel
        : SPDocumentBase, ISearchProgram, ISearchYear
    {
        internal ProgramChangesTravel(IRepository repository, IDbUtilities dbUtilities, DocumentTypeInfo documentTypeInfo)
            : base(repository, dbUtilities, documentTypeInfo)
        { }

        public ProgramChangesTravel WithValues(string programId, int? spSeriesId)
        {
            ProgramId = programId;
        ...
        [SPFieldInfo(SPFieldNames.SpSeriesId, "SPSeriesId", SPFieldType.Text, 1)]
        public int? SPSeriesId { get; private set; }

        public override string FileName => MakeFileName(ProgramId, SPSeriesId);
        ...
        public override string UniqueIdentifiers => "ProgramId;SPSeriesId";

        public override string UniqueValues => string.Format("{0};{1}", ProgramId, SPSeriesId);
        ...
        public override bool Setup(object[] objects)
        {
            ...
            SPSeriesId = Convert.ToInt32(objects[1].ToString());
        ...
        public override string[] ParseFileName(string fileNameToParse)
        {
            ...
            if (!int.TryParse(fileNameParts[2], out int tempSPSeriesId))
            {
